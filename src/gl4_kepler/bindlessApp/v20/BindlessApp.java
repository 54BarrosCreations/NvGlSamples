 ...
 */
package gl4_kepler.bindlessApp.v20;
...
public class BindlessApp extends NvSampleApp {
    ...
    private int maxPerMeshPerUbo;
    private int maxUboSize;
    private int uboAlign;
    private int rbSize;
    private long[] fences;
    private int rbId;
    ...
    public void initRendering(GL4 gl4) {
        ...
        shader = NvGLSLProgram.createFromFiles(gl4, "src/gl4_kepler/bindlessApp/v20/shaders", "v20");
        ...
        perMeshPointer = gl4.glMapBufferRange(GL_UNIFORM_BUFFER, 0, maxUboSize, GL_MAP_WRITE_BIT
                | GL_MAP_PERSISTENT_BIT | GL_MAP_COHERENT_BIT | GL_MAP_INVALIDATE_BUFFER_BIT);

        rbSize = maxPerMeshPerUbo;
        fences = new long[rbSize];
        rbId = 0;
    ...
    private void initBuffers(GL4 gl4) {
        ...
        uboAlign = uniformBufferOffset.get(0);
        ...
        gl4.glBindBuffer(GL_UNIFORM_BUFFER, bufferName.get(Buffer.PER_MESH));
        {
            ...
            maxUboSize = maxUniformBlockSize.get(0);
            gl4.glBufferStorage(GL_UNIFORM_BUFFER, maxUboSize, null, GL_MAP_WRITE_BIT | GL_MAP_PERSISTENT_BIT
                    | GL_MAP_COHERENT_BIT);
            maxPerMeshPerUbo = maxUboSize / uboAlign;
...
//            gl4.glBindBuffer(GL_UNIFORM_BUFFER, 0);
        ...
        for (int i = 0; i < meshes.length; i++) {
            ...
            if (usePerMeshUniforms) {
                ...
                waitBuffer(gl4);
                perMeshPointer.position(((rbId + 1) % rbSize) * uboAlign);
    ...
    private void waitBuffer(GL4 gl4) {
        if (fences[(rbId + 1) % rbSize] > 0) {
            while (true) {
                int waitRet = gl4.glClientWaitSync(fences[(rbId + 1) % rbSize], GL_SYNC_FLUSH_COMMANDS_BIT, 1);
    ...
    private void lockBuffer(GL4 gl4) {
        if (fences[rbId] > 0) {
            gl4.glDeleteSync(fences[rbId]);
        }
        fences[rbId] = gl4.glFenceSync(GL_SYNC_GPU_COMMANDS_COMPLETE, 0);
    }
